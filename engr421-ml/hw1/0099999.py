...
def calculate_score_values(X, pAcd, pCcd, pGcd, pTcd, class_priors):
    ...
    for i in range(num_samples):
        for c in range(num_classes):
            score = np.log(class_priors[c])  # Start with the prior probability of the class
...
def calculate_confusion_matrix(y_truth, scores):
    ...
    y_pred = np.argmax(scores, axis=1) + 1  # Adding 1 to match class labels (assuming class labels start from 1)
