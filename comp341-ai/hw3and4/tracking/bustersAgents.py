...
class KeyboardInference(inference.InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if noisyDistance is not None and busters.getObservationProbability(noisyDistance, trueDistance) > 0:
                allPossible[p] = 1.0
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for g_dist in livingGhostPositionDistributions:
            g_loc.append(max(g_dist, key=g_dist.get))

        distances = {}
        for move in legal:
            for loc in g_loc:
                successorPosition = Actions.getSuccessor(pacmanPosition, move)
                distances[move] = self.distancer.getDistance(successorPosition, loc)
